VERSION 5.00
Begin {C62A69F0-16DC-11CE-9E98-00AA00574A4F} GanID 
   Caption         =   "Tien ich gan ID 6 tham so theo he thong dang toan cua Nhom Toan VD VDC"
   ClientHeight    =   1605
   ClientLeft      =   45
   ClientTop       =   375
   ClientWidth     =   15600
   OleObjectBlob   =   "GanID.frx":0000
   ShowModal       =   0   'False
End
Attribute VB_Name = "GanID"
...
' Gan ID6
    ...
    Dim i As Integer, TextID As String
    ActiveDocument.UndoClear
    If ComboBox1 = "" Or ComboBox2 = "" Or ComboBox3 = "" Or ComboBox4 = "" Or (OptionButton1 = False And OptionButton2 = False And OptionButton3 = False And OptionButton4 = False) Then
    ...
    Else
        Application.ScreenUpdating = False
            ...
            If Left(ComboBox4.Value, 4) = "List" Then
                Exit Sub
            Else
                For i = 4 To Len(ComboBox4.Value)
                    If Mid(ComboBox4.Value, i, 1) = " " Then
                        CdDangtoan = Left(ComboBox4.Value, i - 1)
                        Exit For
                    End If
                Next i
            End If
            ...
            If GanID.OptionButton6 = True Then
                TextID = " %[" & Lop & Mon & Chuong & "-" & CdDangtoan & "-" & Mucdo & "]" & Chr(13)
            Else
                TextID = "[" & Lop & Mon & Chuong & "-" & CdDangtoan & "-" & Mucdo & "]"
            End If
...
' Sua ID6
    ...
    Else
        Application.ScreenUpdating = False
            ...
            If OptionButton4 = True Then Mucdo = OptionButton4.Caption
            If GanID.OptionButton6 = True Then
                NewTextID = " %[" & Lop & Mon & Chuong & "-" & CdDangtoan & "-" & Mucdo & "]" & Chr(13)
            Else
                NewTextID = "[" & Lop & Mon & Chuong & "-" & CdDangtoan & "-" & Mucdo & "]"
            End If
            Selection.Find.ClearFormatting
            Selection.Find.Replacement.ClearFormatting
            With Selection.Find
...
Else
    ...
    With fd
      ...
      If .Show = True Then
        ...
        Selection.InsertFile txtFileName
...
' BT.Pro và ID6
    ...
    Selection.Find.Replacement.ClearFormatting
    With Selection.Find
        .text = "(\[[DH][SH]1[0-2].C[1-6].[1-9].D??.[abcd]\])"
        .Replacement.text = ""
        .Forward = True
        .Wrap = wdFindStop
        .MatchWildcards = True
...
If OptionButton6 = False Then ' Xoa ID thua
    ...
    Selection.Find.Execute Replace:=wdReplaceAll
Else
    If GanID.TextBox1.Value <> "" Then
        ActiveDocument.SaveAs2 GanID.TextBox1.Value, FileFormat:= _
...
On Error Resume Next
    ...
    With Selection.Find
        ...
        .Execute Replace:=wdReplaceAll, Forward:=True
    End With
    ' Xu ly truong hop dac biet (dang toan so 0)
    Selection.HomeKey Unit:=wdStory
    With Selection.Find
        .ClearFormatting
        .text = "(\[??1?.C?.?.)(D0)(.?\])"
